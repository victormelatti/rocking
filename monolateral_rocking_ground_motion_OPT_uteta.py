...
def read_accelerations(filename):
    ...
    accelerations = [s.cell_value(i, 4) * 0.0 for i in range(s.nrows)]
...
def calculate_derivatives(y, t, p_quadro, alpha, time_acceleration_interpolation, gravity):
    ...
    #compute u_theta TODO: can this go in a separate function?
#    global list1
#    global list2
#    global list3
    u_theta=0
...
#        u_theta=0.5*(m*g/(f_m*l_h)+(f_m**3*l_h)/(12*m*g*k_n**2*(theta)**2))
    global u_theta_list, list1, list2, list3
    
    
    if theta<teta_j0 and len(list2)==0:
        u_theta=(B/2-(B**3*k_n*l_h*theta/(12*m*g)))
        list1.append(u_theta)
    if teta_j0 <= theta < teta_jc and len(list3)==0 or (len(list2) > 0 and len(list3) == 0): 
        u_theta=(1/3)*np.sqrt(2*m*g/(k_n*l_h*(theta)))
        list2.append(u_theta)
    if theta>teta_jc or (len(list2) > 0 and len(list3) > 0):
        u_theta=0.5*(m*g/(f_m*l_h)+(f_m**3*l_h)/(12*m*g*k_n**2*(theta)**2))
        list3.append(u_theta)
        if len(list2)==0:
            list2.append(u_theta)
    u_theta_list+=[u_theta]
    print(u_theta)
    derivs = [omega, -p_quadro * (np.sin(alpha - theta)-u_theta/R) + p_quadro * time_acceleration_interpolation(t) * np.cos(alpha - theta) / gravity]
    return derivs
...
def find_rotations(time_array, accelerations, calculation_accuracy):
    ...
    while current_index < len(time_array) and current_index < TEST_STOP:
        ...
        if current_index >= len(time_array) - 1:
            return rotations, time_list_of_lists, skip_forward_list
...
def plot_rotations(time_list_of_lists, rotations_list_of_lists):
    ...
    ax1.plot(t_total, rotation_total, "r-", linewidth=2)
    ...
    ax2.plot(t_total, interpol(t_total), linewidth = 0.6)
    ax2.plot(t_total, [acceleration_of_start_rocking] * len(t_total), "k--", linewidth = 1.5)
    ax2.plot(t_total, [0] * len(t_total), "b", linewidth = 0.6)
    ax2.tick_params(axis="y", labelcolor="blue")
    ax2.set_ylim([interpol(t_total).min(),interpol(t_total).max()+1])
    align_yaxis(ax1, 0, ax2, 0)
    
    # as negative accelerations are of no interest the next plot focus on positive accelerations only
    fig, ax1 = plt.subplots()
    ax1.set_xlabel("Time (s)")
    ax1.set_ylabel("Rotations $\Theta$ (deg)", color="red")
...
rotations, ts, skip_forward_list = find_rotations(t, acc, 1)
