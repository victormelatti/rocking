...
import constants.py and functions.py where are defined the constants and the functions
...
acc = fn.read_accelerations("acceleration_laquila.xlsx", time_array)

interpol = fn.interpolate_time_accelerations(time_array, acc)

rotations, ts, rotation_point = fn.find_rotations(time_array, acc, 1, theta_fully_compressed, theta_fully_cracked, base, cs.INTERFACE_STIFFNESS, cs.COMPRESSIVE_STRENGTH, cs.DEPTH, cs.GRAVITY, mass, radius, alpha, cs.THETA0, cs.OMEGA0, velocity_reduction_coefficient)

t_total, rotation_total = fn.plot_rotations(ts, rotations, rotation_point, acc, cs.T_INC, cs.HALF_BASE, time_array, acceleration_of_start_rocking)
